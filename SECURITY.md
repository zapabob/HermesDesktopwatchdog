...
- **Loopback Default:** By default, the HTTP listener binds exclusively to localhost (`127.0.0.1:9920`). It is not reachable from the external network unless Tailscale (`tsnet`) is explicitly configured with a secure key.
...
- **No Arbitrary Command Execution:** Unlike remote terminals or interpreters, the watchdog only supports executing highly specific predefined commands (launching `Hermes.exe` and spawning `hermes serve` via Python). It rejects and does not implement generic CLI execution paths. Desktop/Backend IPC is **report-only**; `command_request` requires admin token + nonce and an allowlisted `CommandType`.
- **Named Pipe + HTTP IPC:** Local Named Pipe (`\\.\pipe\hermes-watchdog`) is primary on Windows; HTTP loopback is the fallback. Pipe clients cannot execute restart commands.
